...
def teams_have_player_overlap(team_a, team_b):
    players_a = {team_a.player1_id, team_a.player2_id}
    players_b = {team_b.player1_id, team_b.player2_id}
    return bool(players_a.intersection(players_b))


def is_probable_duplicate_match(score_team_a, score_team_b, team_a_id, team_b_id):
...
def create_match(score_team_a, score_team_b, team_a_id, team_b_id):
    ...
    team_a, team_b = get_teams_for_match(team_a_id, team_b_id)

    if not team_a or not team_b:
        return False, "Mindestens eines der Teams existiert nicht.", None

    if teams_have_player_overlap(team_a, team_b):
